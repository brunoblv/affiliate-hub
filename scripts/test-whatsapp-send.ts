 ...
 * sessão já autenticada (scripts/whatsapp-login.ts). Rodar na mesma máquina
 * onde a sessão foi criada (a VPS, não localmente).
 *
 * Uso:
 *   npx tsx scripts/test-whatsapp-send.ts "120363408993059057@g.us"
 */
async function main() {
  const groupJid = process.argv[2];
  if (!groupJid) throw new Error('Uso: npx tsx scripts/test-whatsapp-send.ts "<JID do grupo>"');
