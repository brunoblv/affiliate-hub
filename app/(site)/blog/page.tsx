...
export default async function BlogIndexPage({
  ...
  const where = { status: "PUBLISHED" as const, project: { type: "HOME" as const } };
        ...
        <>
          <Link href={`/blog/${featured.slug}`} className="mt-9 grid grid-cols-1 gap-8 lg:grid-cols-[1.3fr_1fr]">
            ...
            <div className="flex flex-col justify-center">
              ...
              <h2 className="font-heading text-[27px] leading-[1.25] font-semibold text-foreground">{featured.title}</h2>
              {featured.excerpt && <p className="mt-3 text-sm leading-relaxed text-muted-foreground">{featured.excerpt}</p>}
            </div>
          </Link>
            ...
            <div className="mt-11 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {rest.map((post) => (
                <Link key={post.id} href={`/blog/${post.slug}`} className="group block">
                  ...
                  <h3 className="mt-1.5 font-heading text-base font-semibold text-foreground group-hover:underline">{post.title}</h3>
